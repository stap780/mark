(function() {
    ...
    return getClient().then(function(client){
      ...
      return fetch(url, { credentials: 'omit', cache: 'no-cache' })
        ...
        .then(function(data){
          ...
          if (main) { 
            debugLog('favorites:page:direct:main_clearing', { main: main });
            main.innerHTML = ''; 
          }
          ...
          listsData.forEach(function(block){
            ...
            section.setAttribute('data-list-id', String(block.id));
            ...
            grid.setAttribute('data-list-id', String(block.id));
            
            section.appendChild(h2);
            section.appendChild(grid);
            if (main) { main.appendChild(section); }
            
            // Build cards directly from S3 data (no ajaxAPI call needed)
            items.forEach(function(item){
              var card = buildMiniCardFromS3Data(item, block);
  ...
  // Build mini card from S3 data (no ajaxAPI call needed)
  function buildMiniCardFromS3Data(item, listData) {
    ...
    // Add list item with icon for current list
    ...
    listItem.setAttribute('data-added', 'true');
    
    var iconHtml = renderIcon(listData.icon_style || 'icon_one', listData.icon_color || '#999999', true);
  ...
  window.renderFavoritesPageFromS3Direct = renderFavoritesPageFromS3Direct;
  window.buildMiniCardFromS3Data = buildMiniCardFromS3Data;
  window.getClient = getClient;
  window.buildS3ClientListItemsUrl = buildS3ClientListItemsUrl;
  window.buildS3AccountListsUrl = buildS3AccountListsUrl;
  window.fetchListJson = fetchListJson;
  window.updateHeaderInfo = updateHeaderInfo;
  window.createListsContainer = createListsContainer;
