class InsnotifyController < ApplicationController
  ...
  # GET /insnotify/autologin
  # Автологин пользователя из Insnotify
  def autologin
    uid = params[:uid]
    email = params[:email]
    timestamp = params[:timestamp].to_i
    ...
    result = Insnotify.autologin_user(uid: uid, email: email, shop: shop)
  ...
  SECRET = Rails.application.credentials.insnotify[:app_secret]
